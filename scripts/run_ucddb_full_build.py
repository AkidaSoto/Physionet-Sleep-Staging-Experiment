...
def main() -> int:
    ...
    records = discover_ucddb_records(root_dir)
    subjects = [record.record_id for record in records]

    print(f"build_start subjects={len(subjects)} root_dir={root_dir}", flush=True)
    t_all = time.time()
    completed = 0
    failed: list[str] = []
    ...
    print(
        f"build_done completed={completed} failed={len(failed)} elapsed={round(time.time() - t_all, 2)}s",
        flush=True,
    )
    if failed:
