...
def _load_norm_map(path: Path, *, suffix: str) -> dict[str, str]:
    frame = pd.read_parquet(path)
    if frame.empty:
        return {}
    return {
        f"{str(row['feature_name'])}{suffix}": str(row["selected_norm_name"])
    ...
    *,
    ...
    for fold_index, (train_idx, val_idx) in enumerate(splits, start=1):
        ...
        if hsmm_flat.size == len(hsmm_pred_frame):
            ...
            for seq_index, (_, group) in enumerate(raw_pred_frame.groupby(subject_column, sort=False)):
                ordered = group.sort_values("time_seconds", kind="stable") if "time_seconds" in group.columns else group
                tmp = ordered.copy()
                tmp["y_pred"] = np.asarray(val_seq_hsmm[seq_index])[: len(ordered)]
                rebuilt.append(tmp)
...
def main() -> int:
    ...
    from physionet_sleep.experiments.sequence_data import build_mean_context_frame
    ...
    log_path = log_dir / "context_mean_ensemble_5fold_hsmm.log"
    status_path = log_dir / "context_mean_ensemble_5fold_hsmm_status.json"
    ...
    tasks = {
        "staging_plus_shared": specs["staging_plus_shared"],
        "apnea_plus_shared": specs["apnea_plus_shared"],
    }

    out_root = repo_root / "artifacts" / "reference_experiments_context_mean_ensemble_5fold_hsmm"
    out_root.mkdir(parents=True, exist_ok=True)
    overall_status: dict[str, object] = {"status": "running", "tasks": {}, "context_radius": context_radius}
    status_path.write_text(json.dumps(overall_status, indent=2), encoding="utf-8")

    for task_name, spec in tasks.items():
        _log(
            f"task_start name={task_name} target={spec.target_column} base_features={len(spec.feature_columns)}",
            log_path=log_path,
        )
        context_frame = build_mean_context_frame(
            table,
        ...
        context_features = [f"{feature}{suffix}" for feature in spec.feature_columns]
        ...
        norm_map = _load_norm_map(
            repo_root / "artifacts" / "reference_diagnostics_by_task" / ("staging" if task_name.startswith("staging") else "apnea") / "best_feature_norms.parquet",
            suffix=suffix,
        )
        ...
        meta = {
            "target_column": spec.target_column,
            "base_feature_count": len(spec.feature_columns),
            ...
            "norm_source": "best_feature_norms_reused",
            "context_pooling": "mean",
            "context_radius": context_radius,
            "sequence_length": (2 * context_radius) + 1,
        }
